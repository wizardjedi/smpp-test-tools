...
public class App
{
    ...
    private SmppSimulatorSessionHandler serverSession;
    ...
    public void run(String[] args) {
        ...
        config.setBindTimeout(TimeUnit.SECONDS.toMillis(5));
        config.setWriteTimeout(TimeUnit.SECONDS.toMillis(5));
        config.setConnectTimeout(TimeUnit.SECONDS.toMillis(5));
        config.setRequestExpiryTimeout(TimeUnit.SECONDS.toMillis(10));

        client = new DefaultSmppClient();
        try {
            serverSession = sim.pollNextSession(TimeUnit.SECONDS.toMillis(30));
